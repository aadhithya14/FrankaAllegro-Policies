...
class resnet18jeff(nn.Module):
    ...
    def forward(self, x):
        ...
        # logging.info(
        #     f"resnet input: min x: {x.min()}, max x: {x.max()}, mean x: {x.mean()}, std x: {x.std()}"
        # )
        x = self.normalize(x)
        # logging.info(
        #     f"after norm input: min x: {x.min()}, max x: {x.max()}, mean x: {x.mean()}, std x: {x.std()}"
        # )
        out = self.resnet(x)
